// Seeded value/simplex noise shared by CPU systems and (as GLSL source) by shaders.
// Keeping one implementation on both sides means terrain, water and vegetation agree
// on where a hill or a ripple is.
...
export class Noise {
  ...
  /** Fractal sum. octaves>=1, lacunarity 2, gain 0.5 by default. */
  ...
  /** Ridged fbm — good for rock and bark. */
...
export const noise = new Noise(9271);

/**
 * GLSL simplex noise + fbm, injectable into any shader with `${GLSL_NOISE}`.
 * Same visual character as the CPU version above (not bit-identical).
 */
export const GLSL_NOISE = /* glsl */ `
vec3 permute289(vec3 x){ return mod((x*34.0+1.0)*x, 289.0); }
vec4 permute289(vec4 x){ return mod((x*34.0+1.0)*x, 289.0); }

...
async function main() {
  ...
  // Artifact hosting wraps the file in its own <!doctype>/<head>/<body>, so also
  // emit a body-only fragment. Vite hoists the module script into <head>, so
  // gather scripts from the whole document rather than just from <body>.
  ...
  if (bodyMatch) {
    ...
    for (const s of scriptBlocks) markup = markup.split(s).join('');
    const fragment = [
      '<!-- Duckling — generated by tools/bundle-single.mjs. Do not edit. -->',
      ...styles,
      markup.trim(),
      ...scriptBlocks,
    ].join('\n');
    const fragPath = path.join(OUT_DIR, 'duckling.fragment.html');
    await writeFile(fragPath, fragment);
    console.log(`wrote ${fragPath}  (${(Buffer.byteLength(fragment) / 1024).toFixed(0)} KB, body-only)`);
  } else {
    console.warn('WARNING: no <body> found, fragment not written');
  }
